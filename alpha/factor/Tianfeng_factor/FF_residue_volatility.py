...
def FF_3factor_residue(order_book_ids, start_date, end_date):
    ...
    return final.reindex(
        columns=order_book_ids)  # final.groupby('order_book_id').apply(regression_specificity).T.dropna().loc[start_date:,:]


FF_3factor_residue = UserDefinedLeafFactor('FF_3factor_residue', FF_3factor_residue)
