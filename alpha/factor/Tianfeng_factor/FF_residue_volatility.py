...
def FF_3factor_Specificity(order_book_ids,start_date, end_date):
  ...
  #  final['test'] = final.groupby('order_book_id').apply(regression_specificity)
    final = final.dropna()
    final = final.groupby('order_book_id').apply(regression_specificity).apply(pd.Series)
...
a = FF_3factor_Specificity(order_book_ids=stock_pool, start_date='20230101', end_date='20230201')
