...
class Run_Factor():
    ...
    def __init__(self, category, analysis, summary):
        ...
        self.final = pd.DataFrame(index=['第一组减第五组累积收益', '因子累积收益(米筐算法)', '因子收益最大回撤', '因子收益波动率', 'IC_mean',
                                    'IC_std', 'IC_positive', 'IC_negative', 'IC_significance',
                                    'IC_sig_positive', 'IC_sig_negative', 'IC_t_stat', 'IC_p_value',
                                    'IC_skew', 'IC_kurtosis', 'IC_ir'])


    def factor_analysis_selected_files(self, file_name):

   #     try:
        #   start_time = time.time()
        if file_name.endswith('.py'):
            module_name = os.path.splitext(file_name)[0]
            module = importlib.import_module('factor.' + self.directory_name + '.' + module_name)
            # 构造完整的文件路径
            ...
            value_csv = output.OutputResult(pwd=pwd, factor_input=result, factor_name=file_name[:-3],
                                            stock_pool=stock_pool, classification=self.directory_name).factor_value(
                start_date=start_date, end_date=end_date)
            if self.analysis == True:
                temp = output.OutputResult(pwd=pwd, factor_input=result, factor_name=file_name[:-3],
                                           stock_pool=stock_pool, classification=self.directory_name).factor_analysis(
                    df=value_csv)
   ...
   #     except:
   #         print('因子 ' + file_name[:-3] + ' 无法执行，请检查因子定义或网络链接情况')
    ...
    #                    with multiprocessing.Pool(process_number) as pool:
                        # 使用map方法将列表中的每个元素平方并返回结果列表
                 #   pool = multiprocessing.Pool(process_number)
                    pool.map(self.factor_analysis_selected_files, factor_list)
                 #   pool.close()  # 关闭进程池，不再接受新的进程
                #    pool.join()  # 主进程阻塞等待子进程的退出
            #            pool.apply_async(func=self.factor_analysis_selected_files, args=(file_name, directory_name))
        #    if self.summary == True:
        #        self.final.T.to_excel(pwd + r'\result\summary.xlsx')
...
def all_file_search(self,fileName):
    ...
    if summary == True:
        ...
        self.final = self.final.join(temp)
    print('因子 ' + fileName[:-3] + ' 已经完成')

  #  except:
  #     print('因子 ' + fileName[:-3] + '无法执行，请检查定义或网络连接情况')
  #      can_not_execute_list.append(fileName[:-3])
